...
def variable_width_weight_loss(shape, stddev, wl):
...
weight1 = variable_width_weight_loss(shape=[5, 5, 3, 64], stddev=5e-2, wl=0.0)
...
weight2 = variable_width_weight_loss(shape=[5, 5, 64, 64], stddev=5e-2, wl=0.0)
...
weight3 = variable_width_weight_loss(shape=[dim, 384], stddev=0.04, wl=0.004)
bias3 = tf.Variable(tf.constant(0.1, shape=[384]))
local3 = tf.nn.relu(tf.matmul(reshape, weight3) + bias3)

weight4 = variable_width_weight_loss(shape=[384, 192], stddev=0.04, wl=0.004)
bias4 = tf.Variable(tf.constant(0.1, shape=[192]))
local4 = tf.nn.relu(tf.matmul(local3, weight4) + bias4)

weight5 = variable_width_weight_loss(shape=[192, 10], stddev=1 / 192.0, wl=0.0)
...
tensor_info_x = utils.build_tensor_info(image_holder)
